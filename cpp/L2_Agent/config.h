...
namespace AgentConfig
{
    ...
    const std::string NAME{"cpp_l2_agent"};
    // Type of agent.
    const std::string TYPE{"air"};
    // Description of agent.
    const std::string DESCRIPTION{"cpp_l2_agent_example"};
    ...
    const std::vector<std::string> LEVEL{"sensor", "direct execution"};
    // Update rate for sending sensor data (times/second)
    const float UPDATE_RATE{3.0f};

    // JSON structure describing possible tasks (e.g. go-home, move-to)
    const nlohmann::json TASKS_AVAILABLE = {
        {{"name", "go-home"},
...
namespace MqttConfig
{
    ...
    const std::string WARAPS_LISTEN_TOPIC{"/exec/command"};
