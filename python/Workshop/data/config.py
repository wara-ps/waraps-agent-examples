...
class AgentConfig:
    ...
    NAME                    = f"pa_{unique_str}"                                      # Base name of agent. OBS! will be overwritten if -n is used in .yml file                               # Base name of agent. OBS! will be overwritten if -n is used in .yml file
    TYPE                    = "air"                                                   # Type of agent.
    DESCRIPTION             = "L2_agent_example"                                      # Description of agent.
    ...
    TASKS_AVAILABLE         = [{"name": "move-to",
                                "signals": ["$abort", "$enough", "$pause", "$continue"]
                                }]
...
class MqttConfig:
    ...
    WARAPS_BROKER: str     = os.getenv('WARAPS_BROKER')
    WARAPS_PORT: int       = os.getenv('WARAPS_PORT') 
    BASE_VIDEOSERVER_URL: str = "wss://ome.waraps.org:3334/app"
