...
float getHeadingCorrection() {
  float bearing = getBearing2CurrWP();
  float error   = a_pos_data.crs - bearing;  // Use course to calculate the difference
...
float getBearing2CurrWP() {
  GeoPoint aGp;
  aGp.lat = a_pos_data.lat;
  aGp.lon = a_pos_data.lon;
  aGp.alt = a_pos_data.alt;

  return getBearingBetween(aGp, planned_waypoints.ps[planned_waypoints.curr_i]);
}
...
float getDistance2CurrWP() {
  GeoPoint aGp;
  aGp.lat = a_pos_data.lat;
  aGp.lon = a_pos_data.lon;
  aGp.alt = a_pos_data.alt;
  return getDistanceBetween(aGp, planned_waypoints.ps[planned_waypoints.curr_i]);
}
